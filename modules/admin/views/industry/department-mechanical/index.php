...
$this->title = 'Механическая сборка';
$this->params['breadcrumbs'][] = $this->title;



$bad_defects = \app\models\industry\AllDeffect::find()->where(['department_id' => 3])->andWhere(['is_save'=>0])->andWhere(['status'=>1])->sum('count_deffect');
$edit_defects = \app\models\industry\AllDeffect::find()->where(['department_id' => 3])->andWhere(['is_save'=>1])->andWhere(['status'=>1])->sum('count_deffect');
$close_defects = \app\models\industry\AllDeffect::find()->where(['department_id' => 3])->andWhere(['status'=>0])->sum('count_deffect');
    ...
    <section class="content">
        ...
        <div class="row">
            <div class="col-md-3 col-sm-6 col-xs-12">
                <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/all-deffect/index', 'department_id'=>3, 'is_save' => 1, 'status' => 1]);?>">
                    <div class="info-box">
            ...
            <div class="col-md-3 col-sm-6 col-xs-12">
                <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/all-deffect/index', 'department_id'=>3, 'is_save' => 0, 'status' => 1]);?>">
                    <div class="info-box">
            ...
            <div class="col-md-3 col-sm-6 col-xs-12">
                <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/all-deffect/index', 'department_id'=>3, 'status' => 0]);?>">
                    <div class="info-box">
        ...
        <?php if (Yii::$app->session->hasFlash('mechanical_removed')) {?>
            <div class="alert alert-success text-center"><?=Yii::$app->session->getFlash('mechanical_removed');?></div>
        <?php }?>
        <div class="box box-info color-palette-box">
            <div class="box-header">
                <div class="pull-right">
                    <?php if(Yii::$app->user->identity->role == User::ROLE_ADMIN || Yii::$app->user->identity->is_part == User::ROLE_UCHET) {?>
                        <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-mechanical/create'])?>" class="btn btn-primary"><i class="fa fa-plus"></i> Добавить операцию</a>
                    <?php } ?>
                </div>
                <div id="action-links" style="display:none">
                    <?php $form = ActiveForm::begin(['action' => Url::to(['industry/department-mechanical/multi']), 'options' => ['class' => 'busercheck__header _check_user']]); ?>
                    ...
                    <div class="box-body" id="item-block">
                        ...
                        <div style="overflow-x: auto; width: 100%;">
                            ...
                            'rowOptions'   => function ($model, $index, $widget, $grid) {
                                return [
                                    'id' => $model['id'],
                                    'url' => Yii::$app->urlManager->createUrl('admin/industry/department-mechanical/view').'?id='.$model['id']
                                ];
                            },
                            'pager' => [
...
//                                [
//                                    'attribute'=>'new_number_poddon',
//                                    'label'=>'<i class="fa fa-sort"></i> Новый номер поддона',
//                                    'encodeLabel' => false,
//                                ],
                                ...
                                [
                                    ...
                                    'buttons' => [
                                        'update' => function ($url, $model) use ($words) {
                                            ...
                                            return Html::a('<span class="glyphicon glyphicon-pencil"></span>', Yii::$app->urlManager->createUrl(['admin/industry/department-mechanical/create', 'id'=>$model->id]), ['class'=>'btn btn-primary']);
                                        },
                                        'delete' => function ($url, $model) use ($words) {
                                            if (Yii::$app->user->identity->is_remove == false  ) return false;
                                            return Html::a('<span class="glyphicon glyphicon-trash"></span>', Yii::$app->urlManager->createUrl(['admin/industry/department-mechanical/remove', 'id'=>$model->id]), ['class'=>'btn btn-danger remove-object']);
                                        }
                                    ],
                                ]
                            ],
