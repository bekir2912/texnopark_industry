...
class UserController extends Controller {
    ...
    public function actionIndex() {
        ...
        $dataProvider->query->with('image')->andWhere(['role'=>User::ROLE_INDUSTRY_USER])->andWhere(['!=', 'status', 3]);
    ...
    public function actionCreate() {
        $model = new User;
        $model->scenario = User::SIGNUP_INDUSTRY_ADMIN_USER;

        if ($id = Yii::$app->request->get('id')) {
            if ($model = User::find()->with('image')->where(['id'=>$id])->one()) {
                $model->scenario = User::UPDATE_INDUSTRY_ADMIN_USER;
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            if ($model->saveUser(User::ROLE_INDUSTRY_USER, 1)) {
    ...
    public function actionRemove($id) {
        ...
        if ($this->user && ($this->user->role != User::ROLE_INDUSTRY_USER) && $model && $model->removeUser()) {
