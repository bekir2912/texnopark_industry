...
<div class="content-wrapper">
    ...
    <section class="content">
        <?php if (Yii::$app->session->hasFlash('model_removed')) {?>
            <div class="callout callout-success text-center">
                <?=Yii::$app->session->getFlash('model_removed');?>
            </div>
        <?php }?>
        <div class="box box-info color-palette-box">
            <div class="box-header with-border">
                <div class="box-title pull-right" style="font-size: 14px">
                    <?php if (!Yii::$app->request->get('type')) {?>
                        <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/product-model/create'])?>" class="btn btn-primary">
                ...
                <div id="action-links" style="display:none">
                    <?php $form = ActiveForm::begin(['action' => Url::to(['industry/product-model/multi']), 'options' => ['class' => 'busercheck__header _check_user']]); ?>
            ...
            <div class="box-body" id="item-block">
                ...
                <div style="overflow-x: auto; width: 100%;">
                    ...
                    'summary' => "Страница {begin} - {end} из {totalCount} моделей<br/><br/>",
                    'emptyText' => 'Моделей нет',
                    'rowOptions'   => function ($model, $index, $widget, $grid) {
                        return [
                            'id' => $model['id'],
                            'url' => Yii::$app->urlManager->createUrl('admin/industry/product-model/view').'?id='.$model['id']
                        ];
                    },
                    'pager' => [
                        ...
                        [
                            ...
                            'buttons' => [
                                'update' => function ($url, $model) use ($words) {
                                    return Html::a('<span class="glyphicon glyphicon-pencil"></span>', Yii::$app->urlManager->createUrl(['admin/industry/product-model/create', 'id'=>$model->id]), ['class'=>'btn btn-primary']);
                                },
                                'delete' => function ($url, $model) use ($words) {
                                    return Html::a('<span class="glyphicon glyphicon-trash"></span>', Yii::$app->urlManager->createUrl(['admin/industry/product-model/remove', 'id'=>$model->id]), ['class'=>'btn btn-danger remove-object']);
                                }
                            ],
                        ]
                    ],
