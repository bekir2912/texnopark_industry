...
use app\models\industry\DepartmentPlasticSearch;
...
/**
 * DepartmentPlasticController implements the CRUD actions for DepartmentPlastic model.
 */
class DepartmentPlasticController extends Controller
{
    ...
    public function beforeAction($action) {
        ...
        if (($this->user->role == User::ROLE_MODERATOR)) {
            ...
            if (!in_array('department-plastic', $accesses)) {
    ...
    /**
     * Lists all DepartmentPlastic models.
     * @return mixed
     */

    public function actionCheck($id) {
        $model = DepartmentPlastic::findOne($id);
    ...
    {
        $searchModel = new DepartmentPlasticSearch();
        ...
        $models =  \app\models\industry\DepartmentPlastic::find()->select('model_id')->orderBy('model_id')->groupBy('model_id')->all();
        $models_filter = [];
    ...
    /**
     * Displays a single DepartmentPlastic model.
     * @param integer $id
     * @return mixed
    ...
    public function actionReport(){
        ...
        $department_id = 8;
        $department_name = 'b_department_plastic';
...
//        debug($plan_array2);
    ...
    public function actionMulti($id=null)
    {
        ...
        DepartmentPlastic::deleteAll(['id' => $arrays]);
        Yii::$app->session->setFlash('plastic_removed', 'Поддоны успешно удалены');
        return $this->redirect(Yii::$app->request->referrer);
    }


    /**
     * Creates a new DepartmentPlastic model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate($id = null) {
        $model = new DepartmentPlastic();

        if ($id) {
            $model = DepartmentPlastic::find()->where(['id'=>$id])->one();
        }

        if ($model->load(Yii::$app->request->post()) && $model->save()) {

            Yii::$app->session->setFlash('plastic_saved', 'Операция успешно сохранена');
            return $this->redirect(['/admin/industry/department-plastic/view', 'id'=>$model->id]);
        }

        $models = ArrayHelper::map(ProductModel::find()->where(['status'=>1])->andWhere(['department_id' =>8])->all(), 'id', 'name_ru');
    ...
    public function actionStock($id = null){
        $obj = DepartmentPlastic::findOne($id);
        ...
        $notification->type = 'b_department_plastic';
    ...
    public function actionSetDefect($id) {
        ...
        if ($model) {
            $model->is_defect = 0;
            if ($model->save(false)) {
                Yii::$app->session->setFlash('plastic_setted', 'Успешно перемещен в раздел дефектной продукции');
            }
        }
    ...
    public function actionUnsetDefect($id) {
        ...
        if ($model) {
            $model->is_defect = 1;
            if ($model->save(false)) {
                Yii::$app->session->setFlash('plastic_unsetted', 'Усешно возвращен из раздела дефектной продукции');
            }
        }
    ...
    public function actionRemove($id) {
        ...
        if (Yii::$app->user->identity->role != User::ROLE_INDUSTRY_USER  && $model) {
            $model->removeObject();
            Yii::$app->session->setFlash('plastic_removed', 'Операция успешно удалена');
        }

        return $this->redirect(['/admin/industry/department-plastic']);
    }
    ...
    /**
     * Finds the DepartmentPlastic model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return DepartmentPlastic the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = DepartmentPlastic::findOne($id)) !== null) {
            return $model;
        }
