...
<div class="content-wrapper">
    ...
    <section class="content">
        <?php if (Yii::$app->session->hasFlash('notice_removed')) {?>
            <div class="callout callout-success text-center">
                <?=Yii::$app->session->getFlash('notice_removed');?>
            </div>
        <?php }?>
                ...
                <?= GridView::widget([
                    ...
                    'columns' => [
                        ...
                        [
                            ...
                            'label'=>'<i class="fa fa-sort"></i> ID фуры',
                            'encodeLabel' => false,
                        ],
                        [
                        ...
                        [
                            ...
                            'value' => function ($model, $key, $index, $column) {
                                if ($model->status == 1) {
                                    if ($model->noticeTruck->status == 0) {
                                        $truck = 'warning';
                                    } else if ($model->noticeTruck->status == 1) {
                                        $truck = 'success';
                                    } else {
                                        $truck = 'danger';
                                    }
                                    if ($model->noticeTruck->noticeControl->status == 0) {
                                        $control = 'warning';
                                    } else if ($model->noticeTruck->noticeControl->status == 1) {
                                        $control = 'success';
                                    } else {
                                        $control = 'danger';
                                    }
                                    if ($model->noticeTruck->noticeControl->noticeAct->status == 0) {
                                        $act = 'warning';
                                    } else if ($model->noticeTruck->noticeControl->noticeAct->status == 1) {
                                        $act = 'success';
                                    } else {
                                        $act = 'danger';
                                    }
                                    ...
                                    return '
                                        <div class="label label-success">Накладная</div><br/>
                                        <div class="label label-'.$truck.'">Прием грузовика</div><br/>
                                        <div class="label label-'.$control.'">Контроль качества</div><br/>
                                        <div class="label label-'.$act.'">Акт приемки</div>';
                                }
                            },
                        ],
                        [
                                ...
                                'view' => function ($url, $model) {
                                    return '<div class="btn-group"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
                                            ...
                                            <ul class="dropdown-menu pull-left">
                                                <li><a href="'.Yii::$app->urlManager->createUrl(['/admin/notice/waybill', 'id'=>$model->id]).'" class="dropdown-item">Посмотреть</a></li>
                                            </ul>';
                                }
                            ],
                        ]
