...
use app\models\industry\DepartmentPaitingSearch;
...
class DepartmentPaitingController extends Controller
{
    ...
    public function beforeAction($action) {
        ...
        if (($this->user->role == User::ROLE_MODERATOR)) {
            ...
            if (!in_array('department-paiting', $accesses)) {
    ...
    public function actionCheck($id) {
        $model = DepartmentPaiting::findOne($id);
    ...
    public function actionMulti($id=null)
    {
        ...
        DepartmentPaiting::deleteAll(['id' => $arrays]);
        Yii::$app->session->setFlash('paiting_removed', 'Поддоны успешно удалены');
        return $this->redirect(Yii::$app->request->referrer);
    }
    ...
    {
        $searchModel = new DepartmentPaitingSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);


        // Для  фильтрации моделей
        $models =  \app\models\industry\DepartmentPaiting::find()->select('model_id')->orderBy('model_id')->groupBy('model_id')->all();
        $models_filter = [];
    ...
    public function actionCreate($id = null) {
        $model = new DepartmentPaiting();
        $department_id = yii::$app->request->get('department_id');
        $buffer_id = yii::$app->request->get('buffer_id');


        $department = DepartmentStamping::findOne($department_id);
        $buffer = BufferZone::findOne($buffer_id);

        if ($id) {
            $model = DepartmentPaiting::find()->where(['id'=>$id])->one();
        }
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            Yii::$app->session->setFlash('paiting_saved', 'Операция успешно сохранена');
            return $this->redirect(['/admin/industry/department-paiting/view', 'id'=>$model->id]);
        }
    ...
    public function actionReport(){
        ...
        $command3 = $connection->createCommand("SELECT MONTH(p.date), sum(p.value) as `amount`
                                            ...
                                            Where p.status = 1 AND department_id = 2 AND (p.date IS NOT NULL) AND p.date BETWEEN '$month' AND  '$tomorrow_date'
        ...
        $command5 = $connection->createCommand("SELECT MONTH(p.dates), sum(p.count_deffect) as `amount`
                                            FROM b_deffects p
                                            Where p.status = 1 AND department_id = 2 AND (p.dates IS NOT NULL) AND p.dates BETWEEN '$month' AND  '$tomorrow_date'
                                            GROUP BY MONTH(p.dates)");
        ...
        $command6 = $connection->createCommand("SELECT MONTH(p.date), sum(p.value) as `amount`
                                            FROM b_plans_dates p
                                            Where p.status = 1 AND department_id = 2 AND (p.date IS NOT NULL) AND p.date BETWEEN '$month' AND  '$last'
                                            GROUP BY MONTH(p.date)");
        ...
        if($start && $end){
            ...
            $command = $connection->createCommand("SELECT DATE_FORMAT(date,'%Y-%m-%d') as 'dates', sum(value) as 'amount'
                                            FROM b_plans_dates 
                                            Where status = 1 AND date IS NOT NULL
                                            AND department_id = 2 AND date BETWEEN  NOW() - INTERVAL 30 DAY AND NOW() + INTERVAL 30 DAY
                                            GROUP BY DATE_FORMAT(date,'%Y-%m-%d')");
            $plans = $command->queryAll();
...
//        debug($plan_array2);
    ...
    public function actionRemove($id) {
        ...
        if (Yii::$app->user->identity->role != User::ROLE_INDUSTRY_USER  && $model) {
            $model->removeObject();
            Yii::$app->session->setFlash('paiting_removed', 'Операция успешно удалена');
        }

        return $this->redirect(['/admin/industry/department-paiting']);
    }
    ...
    {
        if (($model = DepartmentPaiting::findOne($id)) !== null) {
            return $model;
        }
