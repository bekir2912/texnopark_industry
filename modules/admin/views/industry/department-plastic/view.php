...
<div class="content-wrapper">
    ...
    <section class="content">
        <?php if (Yii::$app->session->hasFlash('plastic_saved')) {?>
            <div class="alert alert-success text-center">
                <?=Yii::$app->session->getFlash('plastic_saved');?>
            </div>
        <?php } ?>
        <?php if (Yii::$app->session->hasFlash('plastic_setted')) {?>
            <div class="alert alert-success text-center">
                <?=Yii::$app->session->getFlash('plastic_setted');?>
            </div>
        <?php }?>
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="box box-info color-palette-box">
                    <div class="box-header">
                        ...
                        <?php if ($model->status == 1 || $model->status == 2 ||$model->status == 0) { ?>
                            <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-plastic/stock', 'id'=>$model->id]);?>" class="btn btn-warning">Добавить на склад</a>
                        <?php }?>
...
<!--                        --><?php //}?>
                        ...
                        <div class="btn-group"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
                            ...
                            <ul class="dropdown-menu pull-left">
                                <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-plastic']);?>" class="dropdown-item">Все операции</a></li>
                                <?php if(Yii::$app->user->identity->is_edit == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-plastic/create', 'id'=>$model->id]);?>" class="dropdown-item">Редактировать</a></li>
                                <?php } ?>
                                <?php if(Yii::$app->user->identity->is_remove == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-plastic/remove', 'id'=>$model->id]);?>" class="dropdown-item" class="remove-object">Удалить</a></li>
                                <?php } ?>
