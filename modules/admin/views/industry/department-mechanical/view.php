...
if($model->id){
    $defect = AllDeffect::find()->where(['dep_id' =>$model->id])->andWhere(['department_id' => 3])->one();
    $buffer = BufferZone::find()->where(['dep_id' =>$model->id])->andWhere(['from_department_id' => 3])->one();
}
                ...
                <div class="box box-info color-palette-box">
                    <div class="box-header">
                        ...
                        <?php if ($model->is_ckeck == 0 &&  (Yii::$app->user->identity->is_part == User::ROLE_OTK || Yii::$app->user->identity->role == User::ROLE_ADMIN)) {?>
                            <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-mechanical/check', 'id'=>$model->id]);?>" class="btn btn-success">Проверку прошел</a>
                        <?php }?>
                        ...
                        <div class="btn-group"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
                            ...
                            <ul class="dropdown-menu pull-left">
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-mechanical/index']);?>" class="dropdown-item">Все операции мех. сборки</a></li>
                                <?php if(Yii::$app->user->identity->is_edit == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-mechanical/create', 'id'=>$model->id]);?>" class="dropdown-item">Редактировать</a></li>
                                <?php } ?>
                                <?php if(Yii::$app->user->identity->is_remove == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-mechanical/remove', 'id'=>$model->id]);?>" class="dropdown-item" class="remove-object">Удалить</a></li>
                                <?php } ?>
...
<!--                                <tr>-->
<!--                                    <td>Новый номер поддона:</td>-->
<!--                                    <td>--><?//=$model->new_number_poddon ? $model->new_number_poddon : '-';?><!--</td>-->
<!--                                </tr>-->
                                <tr>
                                    <td>Кол-во:</td>
