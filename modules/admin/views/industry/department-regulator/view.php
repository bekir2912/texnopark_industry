...
if($model->id){
    $defect = AllDeffect::find()->where(['dep_id' =>$model->id])->andWhere(['department_id' => 8])->one();
...
<div class="content-wrapper">
    ...
    <section class="content">
        <?php if (Yii::$app->session->hasFlash('regulator_saved')) {?>
            <div class="alert alert-success text-center">
                <?=Yii::$app->session->getFlash('regulator_saved');?>
            </div>
        <?php } ?>
        <?php if (Yii::$app->session->hasFlash('regulator_setted')) {?>
            <div class="alert alert-success text-center">
                <?=Yii::$app->session->getFlash('regulator_setted');?>
            </div>
        <?php }?>
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="box box-info color-palette-box">
                    <div class="box-header">
                        <?php if ($model->status == 1 || $model->status == 2 ||$model->status == 0) { ?>
                            <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-regulator/stock', 'id'=>$model->id]);?>" class="btn btn-warning">Добавить на склад</a>
                        <?php }?>
                        ...
                        <div class="btn-group"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
                            ...
                            <ul class="dropdown-menu pull-left">
                                <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-regulator']);?>" class="dropdown-item">Все операции</a></li>
                                <?php if(Yii::$app->user->identity->is_edit == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-regulator/create', 'id'=>$model->id]);?>" class="dropdown-item">Редактировать</a></li>
                                <?php } ?>
                                <?php if(Yii::$app->user->identity->is_remove == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-regulator/remove', 'id'=>$model->id]);?>" class="dropdown-item" class="remove-object">Удалить</a></li>
                                <?php } ?>
