...
use app\models\industry\DepartmentTestSearch;
...
class DepartmentTestController extends Controller
{
    ...
    public function beforeAction($action) {
        ...
        if (($this->user->role == User::ROLE_MODERATOR)) {
            ...
            if (!in_array('department-test', $accesses)) {
    ...
    public function actionCheck($id) {
        $model = DepartmentTest::findOne($id);
    ...
    {
        $searchModel = new DepartmentTestSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);


        // Для  фильтрации моделей
        $models =  \app\models\industry\DepartmentTest::find()->select('model_id')->orderBy('model_id')->groupBy('model_id')->all();
        $models_filter = [];
        ...
        $departments =  DepartmentTest::find()->select('previous_department_id')->orderBy('previous_department_id')->groupBy('previous_department_id')->all();
    ...
    public function actionMulti($id=null)
    {
        ...
        DepartmentTest::deleteAll(['id' => $arrays]);
        Yii::$app->session->setFlash('test_removed', 'Поддоны успешно удалены');
        return $this->redirect(Yii::$app->request->referrer);
    }
    ...
    public function actionCreate($id = null) {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            Yii::$app->session->setFlash('test_saved', 'Операция успешно сохранена');
            return $this->redirect(['/admin/industry/department-test/view', 'id'=>$model->id]);
        }
        $models = ArrayHelper::map(ProductModel::find()->where(['status'=>1])->andWhere(['department_id' =>4])->all(), 'id', 'name_ru');
        $lines = ArrayHelper::map(BLine::find()->where(['status'=>1])->andWhere(['department_id' =>$model->department_id ?$model->department_id: ($department->department_id ? $department->department_id+1 : 4)])->all(), 'id', 'name_ru');
    ...
    public function actionReport(){
        ...
        $department_id = 4;
        $department_name = 'b_department_test';
...
//        debug($plan_array2);
    ...
    public function actionRemove($id) {
        ...
        if (Yii::$app->user->identity->role != User::ROLE_INDUSTRY_USER  && $model) {
            $model->removeObject();
            Yii::$app->session->setFlash('test_removed', 'Операция успешно удалена');
        }

        return $this->redirect(['/admin/industry/department-test']);
    }
    ...
    {
        if (($model = DepartmentTest::findOne($id)) !== null) {
            return $model;
        }
