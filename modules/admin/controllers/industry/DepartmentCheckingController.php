...
use app\models\industry\DepartmentCheckingSearch;
...
/**
 * DepartmentCheckingController implements the CRUD actions for DepartmentChecking model.
 */
class DepartmentCheckingController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function beforeAction($action) {
        ...
        if (($this->user->role == User::ROLE_MODERATOR)) {
            ...
            if (!in_array('department-checking', $accesses)) {
    ...
    /**
     * Lists all DepartmentChecking models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new DepartmentCheckingSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);


        // Для  фильтрации моделей
        $models =  \app\models\industry\DepartmentChecking::find()->select('model_id')->orderBy('model_id')->groupBy('model_id')->all();
        $models_filter = [];
    ...
    /**
     * Displays a single DepartmentChecking model.
     * @param integer $id
     * @return mixed
    ...
    public function actionMulti($id=null)
    {
        ...
        DepartmentChecking::deleteAll(['id' => $arrays]);
        Yii::$app->session->setFlash('checking_removed', 'Поддоны успешно удалены');
    ...
    public function actionReport(){
        ...
        $department_id = 12;
        $department_name = 'b_department_checking';
...
//        debug($plan_array2);
    ...
    /**
     * Creates a new DepartmentChecking model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate($id = null) {
        $model = new DepartmentChecking();

        if ($id) {
            $model = DepartmentChecking::find()->where(['id'=>$id])->one();
        }

        if ($model->load(Yii::$app->request->post()) && $model->save()) {

            Yii::$app->session->setFlash('checking_saved', 'Операция успешно сохранена');
            return $this->redirect(['/admin/industry/department-checking/view', 'id'=>$model->id]);
        }
    ...
    public function actionSetDefect($id) {
        $model = DepartmentChecking::findOne($id);


        if ($model) {
            $model->is_defect = 0;
            if ($model->save(false)) {
                Yii::$app->session->setFlash('checking_setted', 'Успешно перемещен в раздел дефектной продукции');
            }
        }
    ...
    public function actionUnsetDefect($id) {
        ...
        if ($model) {
            $model->is_defect = 1;
            if ($model->save(false)) {
                Yii::$app->session->setFlash('checking_unsetted', 'Усешно возвращен из раздела дефектной продукции');
            }
        }
    ...
    /**
     * Deletes an existing DepartmentChecking model.
    ...
    public function actionRemove($id) {
        ...
        if (Yii::$app->user->identity->role != User::ROLE_INDUSTRY_USER  && $model) {
            $model->removeObject();
            Yii::$app->session->setFlash('checking_removed', 'Операция успешно удалена');
        }

        return $this->redirect(['/admin/industry/department-checking']);
    }
    ...
    /**
     * Finds the DepartmentChecking model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return DepartmentChecking the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = DepartmentChecking::findOne($id)) !== null) {
            return $model;
        }
