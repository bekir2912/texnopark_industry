...
if($model->id){
    if($model->id){
        $defect = AllDeffect::find()->where(['dep_id' =>$model->id])->andWhere(['department_id' => 4])->one();
        $buffer = BufferZone::find()->where(['dep_id' =>$model->id])->andWhere(['from_department_id' => 4])->one();
    }
...
<div class="content-wrapper">
    ...
    <section class="content">
        <?php if (Yii::$app->session->hasFlash('mechanical_saved')) {?>
            <div class="alert alert-success text-center">
                <?=Yii::$app->session->getFlash('mechanical_saved');?>
            </div>
        <?php } ?>
            ...
            <div class="col-sm-12">
                ...
                <div class="box box-info color-palette-box">
                    <div class="box-header">
                        ...
                        <?php if ($model->status == 0 && count($buffer->id) == 0 && $model->is_ckeck  &&  ((Yii::$app->user->identity->is_part == User::ROLE_UCHET) || Yii::$app->user->identity->role == User::ROLE_ADMIN )) {?>
                        ...
                        <?php if ($model->status == 1 && empty(!$buffer->id) &&  (Yii::$app->user->identity->is_part == User::ROLE_UCHET || Yii::$app->user->identity->role == User::ROLE_ADMIN  )) {?>
                        ...
                        <?php if ($model->is_ckeck == 0 &&  (Yii::$app->user->identity->is_part == User::ROLE_OTK || Yii::$app->user->identity->role == User::ROLE_ADMIN)) {?>
                            <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-test/check', 'id'=>$model->id]);?>" class="btn btn-success">Проверку прошел</a>
                        <?php }?>
                        <div class="btn-group"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
                                <span class="fa fa-cog"></span>
                            </button>
                            <ul class="dropdown-menu pull-left">
                                 <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-test/index']);?>" class="dropdown-item">Все операции теста на утечку</a></li>
                                <?php if(Yii::$app->user->identity->is_edit == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-test/create', 'id'=>$model->id]);?>" class="dropdown-item">Редактировать</a></li>
                                <?php } ?>
                                <?php if(Yii::$app->user->identity->is_remove == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-test/remove', 'id'=>$model->id]);?>" class="dropdown-item" class="remove-object">Удалить</a></li>
                                <?php } ?>
