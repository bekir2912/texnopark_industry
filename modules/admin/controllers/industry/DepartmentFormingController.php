...
use app\models\industry\DepartmentFormingSearch;
...
/**
 * DepartmentFormingController implements the CRUD actions for DepartmentForming model.
 */
class DepartmentFormingController extends Controller
{
    ...
    public function beforeAction($action) {
        ...
        if (($this->user->role == User::ROLE_MODERATOR)) {
            ...
            if (!in_array('department-forming', $accesses)) {
    ...
    public function actionCheck($id) {
        $model = DepartmentForming::findOne($id);
    ...
    /**
     * Lists all DepartmentForming models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new DepartmentFormingSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);


        // Для  фильтрации моделей
        $models =  \app\models\industry\DepartmentForming::find()->select('model_id')->orderBy('model_id')->groupBy('model_id')->all();
        $models_filter = [];
    ...
    public function actionReport(){
        ...
        $department_id = 11;
        $department_name = 'b_department_forming';
...
//        debug($plan_array2);
    ...
    /**
     * Displays a single DepartmentForming model.
     * @param integer $id
     * @return mixed
    ...
    public function actionMulti($id=null)
    {
        ...
        DepartmentForming::deleteAll(['id' => $arrays]);
        Yii::$app->session->setFlash('forming_removed', 'Поддоны успешно удалены');
        return $this->redirect(Yii::$app->request->referrer);
    }
    /**
     * Creates a new DepartmentForming model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate($id = null) {
        $model = new DepartmentForming();

        if ($id) {
            $model = DepartmentForming::find()->where(['id'=>$id])->one();
        }

        if ($model->load(Yii::$app->request->post()) && $model->save()) {

            Yii::$app->session->setFlash('forming_saved', 'Операция успешно сохранена');
            return $this->redirect(['/admin/industry/department-forming/view', 'id'=>$model->id]);
        }

        $models = ArrayHelper::map(ProductModel::find()->where(['status'=>1])->andWhere(['department_id' =>11])->all(), 'id', 'name_ru');
    ...
    public function actionStock($id = null){
        $obj = DepartmentForming::findOne($id);
        ...
        $notification->type = 'b_department_forming';
    ...
    public function actionSetDefect($id) {
        ...
        if ($model) {
            $model->is_defect = 0;
            if ($model->save(false)) {
                Yii::$app->session->setFlash('forming_setted', 'Успешно перемещен в раздел дефектной продукции');
            }
        }
    ...
    public function actionUnsetDefect($id) {
        ...
        if ($model) {
            $model->is_defect = 1;
            if ($model->save(false)) {
                Yii::$app->session->setFlash('forming_unsetted', 'Усешно возвращен из раздела дефектной продукции');
            }
        }
    ...
    /**
     * Updates an existing DepartmentForming model.
    ...
    /**
     * Deletes an existing DepartmentForming model.
    ...
    public function actionRemove($id) {
        ...
        if (Yii::$app->user->identity->role != User::ROLE_INDUSTRY_USER  && $model) {
            $model->removeObject();
            Yii::$app->session->setFlash('forming_removed', 'Операция успешно удалена');
        }

        return $this->redirect(['/admin/industry/department-forming']);
    }

    /**
     * Finds the DepartmentForming model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return DepartmentForming the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = DepartmentForming::findOne($id)) !== null) {
            return $model;
        }
