...
if($model->id){
    $defect = AllDeffect::find()->where(['dep_id' =>$model->id])->andWhere(['department_id' => 5])->one();
    $buffer = BufferZone::find()->where(['dep_id' =>$model->id])->andWhere(['from_department_id' => 5])->one();
}



$this->title = 'Калибровка №' . ' ' . $model->id; ;
...
<div class="content-wrapper">
    ...
    <section class="content">
        <?php if (Yii::$app->session->hasFlash('sizing_saved')) {?>
            <div class="alert alert-success text-center">
                <?=Yii::$app->session->getFlash('sizing_saved');?>
            </div>
        <?php } ?>
                ...
                <div class="box box-info color-palette-box">
                    <div class="box-header">
                        ...
                        <?php if ($model->is_ckeck == 0 &&  (Yii::$app->user->identity->is_part == User::ROLE_OTK || Yii::$app->user->identity->role == User::ROLE_ADMIN)) {?>
                            <a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-sizing/check', 'id'=>$model->id]);?>" class="btn btn-success">Проверку прошел</a>
                        <?php }?>
                        ...
                        <div class="btn-group"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
                            ...
                            <ul class="dropdown-menu pull-left">
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-sizing/index']);?>" class="dropdown-item">Все операции калибровки</a></li>
                                <?php if(Yii::$app->user->identity->is_edit == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-sizing/create', 'id'=>$model->id]);?>" class="dropdown-item">Редактировать</a></li>
                                <?php }?>
                                <?php if(Yii::$app->user->identity->is_remove == true  ) { ?>
                                    <li><a href="<?=Yii::$app->urlManager->createUrl(['/admin/industry/department-sizing/remove', 'id'=>$model->id]);?>" class="dropdown-item" class="remove-object">Удалить</a></li>
                                <?php }?>
                            </ul>
                        </div>
