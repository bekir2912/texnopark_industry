...
class ModeratorController extends Controller{
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            if ($model->saveUser(User::ROLE_MODERATOR, 1)) {
                Yii::$app->session->setFlash('moderator_save', 'Сотрудник успешно сохранен');
            }
        ...
        $moderator_check = ArrayHelper::map(ModeratorAccess::find()->where(['user_id'=>Yii::$app->request->get('id')])->all(), 'moderator_id', 'moderator_id');
    ...
    public function actionRemove($id) {
        ...
        if ($this->user && ($this->user->role != User::ROLE_USER) && $model && $model->removeUser()) {
            Yii::$app->session->setFlash('moderator_removed', 'Сотрудник успешно удален');
        }
